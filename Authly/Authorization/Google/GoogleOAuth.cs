...
namespace Authly.Authorization.Google
{
    /// <summary>
    /// Extension methods for registering Google OAuth authentication services in the dependency injection container
    /// </summary>
    public static class GoogleOAuthExtension
    {
        /// <summary>
        /// Adds Google OAuth authentication services to the service collection
        /// </summary>
        /// <param name="services">The service collection to add services to</param>
        /// <returns>The service collection for method chaining</returns>
        public static IServiceCollection AddGoogleOAuth(this IServiceCollection services)
        {
            services.AddScoped<IGoogleOAuth, GoogleOAuth>();
            return services;
        }
    }

    /// <summary>
    /// Interface for Google OAuth 2.0 authentication with PKCE (Proof Key for Code Exchange) support
    /// </summary>
    public interface IGoogleOAuth
    {
        /// <summary>
        /// Indicates whether Google OAuth authentication is enabled in the application configuration
        /// </summary>
        bool IsEnabled { get; init; }

        /// <summary>
        /// Handles the initiation of Google OAuth login flow with PKCE parameters
        /// </summary>
        ...
        /// <summary>
        /// Handles the Google OAuth callback after user authorization, including token exchange and user sign-in
        /// </summary>
        /// <param name="context">The HTTP context for the current request</param>
        /// <returns>A task representing the asynchronous operation</returns>
        ...
        /// <summary>
        /// Determines if the current request is a Google OAuth callback request
        /// </summary>
        /// <param name="context">The HTTP context to check</param>
        /// <returns>True if this is a Google OAuth callback request, false otherwise</returns>
        bool IsCallback(HttpContext context);
        
        /// <summary>
        /// Determines if the current request is a Google OAuth login initiation request
        /// </summary>
        /// <param name="context">The HTTP context to check</param>
        /// <returns>True if this is a Google OAuth login request, false otherwise</returns>
        bool IsLogin(HttpContext context);
        
        /// <summary>
        /// Determines if the current request is any Google OAuth related request (login or callback)
        /// </summary>
        /// <param name="context">The HTTP context to check</param>
        /// <returns>True if this is any Google OAuth request, false otherwise</returns>
        bool IsRequest(HttpContext context);
    }

    /// <summary>
    /// Google OAuth 2.0 authentication service implementing PKCE (Proof Key for Code Exchange) flow
    ...
    public class GoogleOAuth : IGoogleOAuth
    {
        ...
        /// <summary>
        /// Initializes a new instance of the GoogleOAuth class with required dependencies
        /// </summary>
        /// <param name="configuration">Application configuration for reading Google OAuth settings</param>
            ...
            ISessionTrackingService sessionTrackingService)
        {
            ...
            IsEnabled = configuration.GetValue<bool>("Application:ExternalAuth:EnableGoogle");
        ...
        /// <summary>
        /// The provider name identifier for Google OAuth
        /// </summary>
        public const string ProviderName = "Google";
        
        /// <summary>
        /// The URI path for initiating Google OAuth login flow
        /// </summary>
        public const string LoginUri = "/google-login";
        
        /// <summary>
        /// The URI path for receiving Google OAuth callbacks
        /// </summary>
        public const string CallbackUri = "/google/oauth2/callback";
        ...
        /// <summary>
        /// Handles Google OAuth login initiation by generating PKCE parameters and redirecting to Google OAuth
        ...
        public async Task HandleLoginAsync(HttpContext context)
        {
            try
            {
                ...
                if (_securityService.IsIpBanned(ipAddress))
                {
                    ...
                    _appLogger.LogWarning("GoogleLogin", $"IP {ipAddress} is banned until {banEnd}, blocking OAuth login attempt");
                ...
                if (!IsEnabled)
                {
                    _appLogger.LogWarning("GoogleLogin", "Google authentication is disabled in configuration");
                ...
                {
                    _appLogger.LogError("GoogleLogin", "Google OAuth Client ID is not configured");
                ...
                // Construct Google OAuth authorization URL with PKCE
                ...
                _appLogger.Log("GoogleLogin", "Redirecting to Google OAuth authorization server");
            ...
            {
                _appLogger.LogError("GoogleLogin", $"Error during Google OAuth initiation: {ex.Message}", ex);
                context.Response.Redirect("/login?error=google_login_failed");
            }
        }

        /// <summary>
        /// Handles Google OAuth callback by validating state, exchanging authorization code for tokens using PKCE,
        ...
        public async Task HandleLoginCallback(HttpContext context)
        {
            try
            {
                ...
                {
                    var banEnd = _securityService.GetIpBanEndTime(ipAddress);
                    _appLogger.LogWarning("GoogleOAuth", $"IP {ipAddress} is banned until {banEnd}, blocking OAuth callback");
                ...
                {
                    _appLogger.LogWarning("GoogleOAuth", $"Google OAuth error: {error}");
                ...
                {
                    _appLogger.LogWarning("GoogleOAuth", "Missing required OAuth callback parameters");
                ...
                {
                    _appLogger.LogError("GoogleOAuth", $"OAuth state mismatch - Session: '{sessionState}', Received: '{state}'");
                ...
                {
                    _appLogger.LogError("GoogleOAuth", "PKCE code verifier missing from session");
                ...
                {
                    _appLogger.LogError("GoogleOAuth", $"Token exchange failed: {tokenContent}");
                ...
                {
                    _appLogger.LogError("GoogleOAuth", "No access token received from Google");
                ...
                {
                    _appLogger.LogError("GoogleOAuth", $"Failed to retrieve user info: {userInfoContent}");
                    context.Response.Redirect("/login?error=userinfo_failed");
                    return;
                }

                _appLogger.LogInfo("GoogleOAuth", "Successfully retrieved user information from Google");
                ...
                {
                    _appLogger.LogWarning("GoogleOAuth", "Missing required user information from Google");
                ...
                {
                    _appLogger.LogError("GoogleOAuth", $"User registration is disabled: {email}");
                    context.Response.Redirect("/login?error=user_creation_failed");
                    return;
                }
                else
                {
                    // Create new user account for Google OAuth user
                    user = new User
                    {
                    ...
                    if (!createResult)
                    {
                        _appLogger.LogError("GoogleOAuth", $"Failed to create user account for Google login: {email}");
                        context.Response.Redirect("/login?error=user_creation_failed");
                        return;
                    }

                    _appLogger.Log("GoogleOAuth", $"New user account created for Google login: {email}");
                }
                ...
                _appLogger.LogInfo("GoogleOAuth", $"User {email} successfully signed in via Google OAuth from IP: {ipAddress}");
            ...
            {
                _appLogger.LogError("GoogleOAuth", $"Error during Google OAuth callback processing: {ex.Message}", ex);
